...
describe('sanitizeFilename', () => {
  ...
  it('handles filenames starting or ending with a period correctly', () => {
    ...
    expect(sanitizeFilename(inputStartingWithDot, defaultOptions)).toBe(
      expectedStartingWithDot
    )

    const inputEndingWithDot = 'file.'
    const expectedEndingWithDot = 'file'
    expect(sanitizeFilename(inputEndingWithDot, defaultOptions)).toBe(
      expectedEndingWithDot
    )
  })
  ...
  it('throws when replacement contains reserved characters', () => {
    ...
    const options = { replacement: '?' }
    expect(() => sanitizeFilename(input, options)).toThrow(
