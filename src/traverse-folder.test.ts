...
describe('traverseFolder', () => {
  it('should traverse all files and directories asynchronously', async () => {
    ...
    expect(handlerMock).toHaveBeenCalledWith(path.join('/mock/path', 'file1.txt'), mockFiles[0]);
    expect(handlerMock).toHaveBeenCalledWith(path.join('/mock/path', 'file2.txt'), mockFiles[1]);
    expect(handlerMock).toHaveBeenCalledWith(path.join('/mock/path', 'subdir'), mockFiles[2]);
  });
  ...
  it('should stop traversal when handler returns true', async () => {
    ...
    const handlerMock = vi.fn().mockImplementation((filePath: string) => filePath.endsWith('file2.txt'));
